...
public class Writer {
    ...
    public static DataBank fillBank(DataEvent event, List<Particle> partlist, String bankName) {
        ...
        for (int ii = 0; ii < partlist.size(); ii++) {
            ...
            partBank.setFloat("umass",i, (float) partlist.get(ii).getUmass());
            ...
            if(partlist.get(ii).getDaughters().size()>0) {
                ...
                for(int j =0; j<partlist.get(ii).getDaughters().size(); j++) {
                    ...
                    partBank.setFloat("mass",i, (float) partlist.get(ii).getDaughters().get(j).getMass());
                    partBank.setFloat("umass",i, (float) partlist.get(ii).getDaughters().get(j).getRecMass());
